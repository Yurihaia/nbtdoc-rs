use nom::{
    ...
    combinator::{cut, map, map_res, not, opt, peek},
...
pub fn root<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, NbtDocFile, E> {
    map(
        many0(alt((
            ...
            map(preceded(sp, terminated(enum_def, sp)), RootItem::Enum),
            map(preceded(sp, terminated(use_def, sp)), RootItem::Use),
            map(
                preceded(sp, terminated(describe_def, sp)),
                RootItem::Describe,
            ),
            map(preceded(sp, terminated(mod_def, sp)), |v| {
                RootItem::Mod(String::from(v))
            }),
            map(preceded(sp, terminated(inject_def, sp)), RootItem::Inject),
        ))),
        |v| {
    ...
    F: Fn(char) -> bool,
{
    move |i: &'a str| {
        if f(i
            .chars()
            .next()
            .ok_or_else(|| nom::Err::Error(make_error(i, ErrorKind::Eof)))?)
        {
            Ok((&i[1..], ()))
        } else {
...
fn range<'a, F, T, E: ParseError<&'a str>>(
    f: F,
) -> impl Fn(&'a str) -> IResult<&'a str, Range<T>, E>
where
    F: Fn(&'a str) -> IResult<&'a str, T, E> + Copy,
{
    alt((
        map(
            separated_pair(terminated(f, sp), tag(".."), preceded(sp, f)),
            |(l, u)| Range::Both(l, u),
        ),
        map(preceded(tag(".."), preceded(sp, f)), Range::High),
        map(terminated(terminated(f, sp), tag("..")), Range::Low),
        map(f, Range::Single),
    ))
}
...
fn float_c<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, f32, E> {
    ...
    match o.parse::<f32>() {
        ...
        Err(_) => IResult::<&'a str, f32, E>::Err(nom::Err::Error(E::from_error_kind(
            i,
...
fn double_c<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, f64, E> {
    ...
    match o.parse::<f64>() {
        ...
        Err(_) => IResult::<&'a str, f64, E>::Err(nom::Err::Error(E::from_error_kind(
            i,
            nom::error::ErrorKind::Float,
        ))),
    }
...
fn number_type<'a, E: ParseError<&'a str>>(
    ...
    preceded(tag(ntype), move |i| match ntype {
        "byte" => map(
...
fn field_type<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, FieldType, E> {
    alt((
        ...
        map(
            pair(
                delimited(pair(tag("["), sp), cut(field_type), pair(sp, tag("]"))),
                opt(preceded(
                    tuple((sp, tag("@"), sp)),
                    cut(natural_range::<i32, E>),
                )),
            ),
        ...
        map(
            pair(
                ...
                delimited(pair(tag("["), sp), field_path, pair(sp, tag("]"))),
            ),
            |(d, p)| FieldType::IndexType { target: d, path: p },
        ),
        map(
            preceded(
                pair(tag("id"), sp),
                delimited(pair(tag("("), sp), mc_ident, pair(sp, tag(")"))),
            ),
            FieldType::IdType,
        ),
        ...
        map(
            delimited(
                pair(tag("("), sp),
                separated_list(tuple((sp, tag("|"), sp)), field_type),
                pair(sp, tag(")")),
            ),
            FieldType::OrType,
        ),
...
fn field_path<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, Vec<FieldPath>, E> {
    map(separated_nonempty_list(tag("."), key), |v| {
        v.into_iter()
            .map(|x| match x {
...
fn compound_def<'a, E: ParseError<&'a str>>(
    i: &'a str,
) -> IResult<&'a str, (String, CompoundDef), E> {
    map(
        tuple((
            terminated(doc_comment, sp),
            preceded(
                pair(tag("compound"), sp1),
                cut(tuple((
                    ident,
                    opt(preceded(
                        tuple((sp1, tag("extends"), sp1)),
                        cut(alt((
                            map(
                                pair(
                                    terminated(mc_ident, sp),
                                ...
                                |(d, p)| CompoundSuper::Registry { target: d, path: p },
                            ),
        ...
        pair(
            ...
            preceded(
                ...
                preceded(
                    tuple((
                        sp,
                        delimited(pair(tag("("), sp), tag(etype), pair(sp, tag(")"))),
                        sp,
                    )),
                    cut(pair(
...
fn mc_ident<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, Identifier, E> {
    map(
        ...
        |(v, _, p)| Identifier::new(String::from(v), p.join("/")),
    )(i)
}
            ...
            cut(pair(
                ...
                preceded(
                    ...
                    terminated(
                        opt(delimited(
                            pair(tag("["), sp),
                            cut(separated_list(tuple((sp, tag(","), sp)), mc_ident)),
                            pair(sp, tag("]")),
                        )),
                        pair(sp, tag(";")),
                    ),
...
fn use_def<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, (bool, Vec<PathPart>), E> {
    map(
        pair(
            ...
            preceded(
                pair(tag("use"), sp1),
                cut(terminated(ident_path, pair(sp, tag(";")))),
            ),
        ),
        |(x, v)| (x.is_some(), v),
    )(i)
...
fn mod_def<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, &'a str, E> {
    preceded(
        pair(tag("mod"), sp1),
        cut(terminated(ident, pair(sp, tag(";")))),
    )(i)
}
                            ...
                            separated_list(
                                ...
                                separated_pair(
                                    ...
                                    move |i| match etype {
                                        ...
                                        "double" => map(double_c, EnumVal::Double)(i),
                                        "string" => {
                                            map(quoted_str, |v| EnumVal::String(String::from(v)))(i)
                                        }
                                        _ => panic!("Something is very wrong"),
                                    },
                                ),
...
mod tests {
    ...
    mod field_type_tests {
        ...
        fn number() {
            assert_eq!(
                field_type::<NVE>("double"),
                Ok(("", FieldType::NumberType(NumberPrimitiveType::Double(None))))
            );
        }
        ...
        fn number_range_both() {
            assert_eq!(
                field_type::<NVE>("int @ 0..100"),
                Ok((
                    "",
                    FieldType::NumberType(NumberPrimitiveType::Int(Some(Range::Both(0, 100))))
                ))
            );
        }
        ...
        fn number_range_upper() {
            assert_eq!(
                field_type::<NVE>("float @ ..0.15"),
                Ok((
                    "",
                    FieldType::NumberType(NumberPrimitiveType::Float(Some(Range::High(0.15))))
                ))
            );
        }
        ...
        fn number_range_lower() {
            assert_eq!(
                field_type::<NVE>("long @ -7.."),
                Ok((
                    "",
                    FieldType::NumberType(NumberPrimitiveType::Long(Some(Range::Low(-7))))
                ))
            );
        }
        ...
        fn number_range_single() {
            assert_eq!(
                field_type::<NVE>("short @ 0"),
                Ok((
                    "",
                    FieldType::NumberType(NumberPrimitiveType::Short(Some(Range::Single(0))))
                ))
            );
        }
        ...
        fn array_simple() {
            assert_eq!(
                ...
                Ok((
                    "",
                    FieldType::ArrayType(NumberArrayType::Int {
                        value_range: None,
                        len_range: None
                    })
                ))
        ...
        fn array_value_range() {
            assert_eq!(
                field_type::<NVE>("long @ ..20[]"),
                Ok((
                    "",
                    FieldType::ArrayType(NumberArrayType::Long {
        ...
        fn array_both_range() {
            assert_eq!(
                field_type::<NVE>("byte @ -20..30[] @ 4.."),
                Ok((
                    "",
                    FieldType::ArrayType(NumberArrayType::Byte {
        ...
        fn named_simple() {
            assert_eq!(
                field_type::<NVE>("FooBar"),
                Ok((
                    "",
                    FieldType::NamedType(vec![PathPart::Regular(fs!("FooBar"))])
                ))
            )
        }
        ...
        fn named_path() {
            assert_eq!(
                field_type::<NVE>("super::module::FooBar"),
                Ok((
                    "",
                    FieldType::NamedType(vec![
                        PathPart::Super,
        ...
        fn id_type() {
            assert_eq!(
                field_type::<NVE>("id(minecraft:item)"),
                Ok(("", FieldType::IdType(id!("minecraft", "item"))))
            )
        }
        ...
        fn path_index() {
            assert_eq!(
                field_type::<NVE>("minecraft:item[id.super.field]"),
                Ok((
                    "",
                    FieldType::IndexType {
                        path: vec![
        ...
        fn or_type() {
            assert_eq!(
                field_type::<NVE>("(int | boolean | byte @ 0..1)"),
                Ok((
                    "",
                    FieldType::OrType(vec![
                        FieldType::NumberType(NumberPrimitiveType::Int(None)),
                        FieldType::BooleanType,
                        FieldType::NumberType(NumberPrimitiveType::Byte(Some(Range::Both(0, 1))))
                    ])
                ))
            )
        }
    ...
    mod describe_def_tests {
        ...
        fn simple() {
            assert_eq!(
                describe_def::<NVE>(
                    "MyCompound describes minecraft:item[minecraft:stick, minecraft:tnt];"
                ),
                Ok((
                    "",
                    (
                        ...
                        DescribeDef {
                            ...
                            targets: Some(
                                vec![id!("minecraft", "stick"), id!("minecraft", "tnt"),]
                            )
                        }
                    )
                ))
    ...
    mod compound_def_tests {
        ...
        fn extend_registry() {
            assert_eq!(
                compound_def::<NVE>(
                    r#"compound Foo extends namespace:registry[id] {
                    ...
                    (
                        ...
                        CompoundDef {
                            ...
                            extend: Some(CompoundSuper::Registry {
                                ...
                                path: vec![FieldPath::Child(fs!("id"))]
                            })
                        }
                    )
                ))
                        ...
                        CompoundDef {
                            ...
                            fields: vec![(
                                ...
                                Field {
                                    ...
                                    optional: false,
                                    field_type: FieldType::NumberType(NumberPrimitiveType::Byte(
                                        None
                                    ))
                                }
                            )],
        ...
        fn extends() {
            assert_eq!(
                compound_def::<NVE>(
                    r#"compound Foo extends some::module::Bar {
					field: boolean
				}"#
                ),
                Ok((
        ...
        assert_eq!(
            ...
            Ok((
                ...
                NbtDocFile {
                    ...
                    compounds: vec![
                        ...
                        (
                            ...
                            CompoundDef {
                                ...
                                extend: Some(CompoundSuper::Compound(vec![PathPart::Regular(
                                    fs!("Breedable")
                                )]))
                            }
                        ),
                    ...
                    describes: vec![
                        (
                            vec![PathPart::Regular(fs!("Breedable"))],
                            DescribeDef {
                        ...
                        (
                            vec![PathPart::Regular(fs!("Sheep"))],
                            DescribeDef {
                                describe_type: id!("minecraft", "entity"),
                                targets: Some(vec![id!("minecraft", "sheep"),])
                            }
                        ),
                        (
                            vec![PathPart::Regular(fs!("Panda"))],
                            DescribeDef {
                                describe_type: id!("minecraft", "entity"),
                                targets: Some(vec![id!("minecraft", "panda"),])
                            }
                        )
                    ],
                    injects: vec![]
